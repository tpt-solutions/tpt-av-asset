...
///
/// Every source frame is decoded through the kinetix stack, aspect-fit into
/// the profile's target resolution, and re-encoded with
/// `tpt-kinetix-lossless` into the TPT proxy stream. When the profile sets
/// a target frame rate, frames are decimated (never duplicated) to
/// approach it.
///
/// # Errors
/// Returns [`AssetError::Cancelled`] when `progress` is cancelled (partial
/// output cleanup is the caller's responsibility — see
/// [`crate::generator::ProxyGenerator`]), [`AssetError::Codec`] for
/// decode/encode failures, and [`AssetError::Validation`] for a bad
/// profile.
pub fn generate(
    source_path: &Path,
    output_path: &Path,
    ...
    let mut source = tpt_av_asset_cache::video::open_video(source_path)?;
    ...
    loop {
        ...
        if keep {
            sink.write_resized(&tpt_av_asset_cache::video::RgbaFrame {
                index: frame.index,
